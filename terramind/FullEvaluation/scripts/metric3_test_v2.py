"""
متركس ٣ — التجربة الثانية بالنص الحقيقي
نقارن: الحالي  مقابل  claim-level باتجاهين
...
# ---------- النص الحقيقي (مطابق للتقرير) ----------
cases = {
  "G014": {  # تباعد محتوى حقيقي -> نبيه يظل منخفض
  ...
  "G045": {  # محتوى صحيح طويل -> نبي نرفعه
...
def claim_recall(m,g,p):
    """كل ادعاء ذهبي: لقى مقابل؟ (المعنى موجود)"""
...
def claim_precision(m,g,p):
    """كل ادعاء مولّد: له أصل ذهبي؟ (يعاقب الزايد)"""
    gc,pc=claims(g),claims(p)
    ge,pe=m.encode(gc),m.encode(pc)
...
for qid,d in cases.items():
    print(f"\n[{qid}]  (المطلوب: ",
          "يظل منخفض)" if qid=="G014" else "يرتفع فوق 0.7)")
    print(f"  الحالي (نص كامل):        {d['old']:.3f}")
    f1,r,pr = bidir_f1(mini,d['golden'],d['generated'])
    print(f"  claim recall  (معنى):    {r:.3f}")
    print(f"  claim precision (زايد):  {pr:.3f}")
    print(f"  claim-F1 (الجديد):       {f1:.3f}")
print("\n"+"="*60)
print("نبي: G045 يرتفع، G014 يظل منخفض، والفجوة بينهم تتّسع")
