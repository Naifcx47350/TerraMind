...
- **General RAG package** (`terramind/rag/general/`) is the active pipeline: load PDFs with `pypdf`, chunk, embed (`text-embedding-3-small`), store in Chroma, retrieve with MMR + topic boost + lexical rerank.
