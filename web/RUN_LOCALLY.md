# Run web locally (TerraMind)

## Path convention

Replace **`<repo-root>`** with wherever you cloned TerraMind (the folder that contains `core/`, `run_dev.py`, and `web/`).
...
After the first `cd <repo-root>`, other steps use **relative** folders (`web/`, `web/frontend-react/`).
...
Requires `npm install` once in `<repo-root>/web/frontend-react`.
...
| -------- | ---------------------------------------------- | -------- |
| 1        | TerraMind Model API (`core.api.app`) | **8001** |
| 2        | web API (`uvicorn app.main`)             | **8000** |
...
```powershell
python -m core.rag.product.cli --reset    # product catalog Excel
python -m core.rag.general.cli --reset   # general agriculture PDFs (required for general/auto)
```
...
| ------------- | -------------------------------------- | -------------------------- |
| `auto_rag` (default) | `core/models/auto_rag.py` + `router.py` | Picks product, general, or **base LLM** per question |
| `general_rag` | `core/models/general_rag.py` → `core/rag/general/` | PDFs in `data/raw/documents/` |
| `product_rag` | `core/models/product_rag.py` → `core/rag/product/` | `data/raw/product_catalog/translated/product_catalog_en.xlsx` |
| `base_llm`    | `core/models/base_llm.py`         | OpenAI only (no retrieval) |
...
## 2. web backend (FastAPI) — terminal 2

**Important:** run `uvicorn` from **`<repo-root>/web`**, not from `<repo-root>` alone.  
...
**Product RAG connected to the website** (recommended after the model API is running):

The API now defaults to RAG (`use_mock=false`, `RAG_SERVICE_URL=http://localhost:8001/query`) even without a `.env` file.

Optional — create `web/.env` explicitly:
...
OpenAI key: `<repo-root>/.env` or `<repo-root>/web/.env` with `OPENAI_API_KEY=...` (read by `core.rag.product` / `core.api.app`).

**Restart web uvicorn after any `.env` or config change.**
...
**Important:** `package.json` is in `frontend-react/`, not in `web/`.  
If you run `npm install` from `web` only, you get `ENOENT ... package.json`.

```powershell
cd <repo-root>/web/frontend-react
nvm use 24.15.0
npm install
npm run dev
```
...
1. Terminal 1: Model API on **8001** (`uvicorn core.api.app:app --reload --port 8001`) — `/health` OK
2. Terminal 2: web `uvicorn` on **8000** — `.env` has `RAG_SERVICE_URL` and `USE_MOCK=false`
...
| Problem                                                   | What to do                                                                                     |
| --------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| “Starting TerraMind…” overlay stays a long time           | Gateway :8000 not up yet — wait or start `uvicorn app.main:app` in `web/`; in Docker wait for healthy `gateway` |
| Cannot connect to API / Vite `ECONNREFUSED` on `/api/ask` | Start `uvicorn` **inside `web/`** on port 8000; confirm http://localhost:8000/api/health |
| `Could not import module "app.main"`                      | Wrong folder — `cd <repo-root>/web` before `uvicorn`                                       |
| Frontend won’t start                                      | `cd <repo-root>/web/frontend-react`, then `nvm use`, `npm install`, `npm run dev`          |
...
For offline UI demos without RAG, set in `web/.env`:
...
Model API must be running on **8001** with indexes built (`python -m core.rag.product.cli --reset` and `python -m core.rag.general.cli --reset`).
...
| Gateway API tests           | `cd <repo-root>/web` then `pytest tests/ -v` |
| Production build (frontend) | `cd <repo-root>/web/frontend-react` then `npm run build` |
...
**Terminal 3 — UI** (must be `frontend-react`, not `web`)
