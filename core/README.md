# core/ — Model API, Models, and RAG

`core/` is the backend package behind the TerraMind model service. It owns the
...
the web gateway first:

```text
React UI (:3000) → web API (:8000) → terramind Model API (:8001)
```
...
web gateway and React UI. In Docker, the same app runs inside the `model-api`
container.
...
Model modes are registered in `core/models/__init__.py`. Each mode returns the
...
- The `model-api` image contains `core/`; corpus files are mounted at runtime.
...
- `core/` owns model execution and RAG logic.
- `web/app/` owns the user-facing gateway API and browser key sync.
- `web/frontend-react/` owns UI state, rendering, sessions, and user controls.
